<?php
    $meta_description = '日報を通して従業員の働き方を可視化し、HR業務を効率化、社内のコミュニケーションを活性化することを可能にするクラウドツールNippoの紹介です。インドネシアのITコンサルティング会社LOGIQUEが開発販売している日報管理ツールです。客観的で数字に基づいた人事評価を行うことを可能にします。';
    $meta_keyword = 'Nippo, daily reporting tool, web-based application, cost effectiveness, assists managers, tool for managers, productivity tool, employee productivity, daily work tool, staff evaluation, employee evaluation, Man-Hour cost management, staff cost management, Remote worker, remote work, Internal communication, human resources tool, management tool';

    $og_title = '日報でHR業務を効率化＆生産性向上 | Nippo';
    $og_url = 'https://nippo.id';
    $og_image = 'https://nippo.id/assets/img/ogp/ogp-index.jpg';
    $og_description = '日報を通して従業員の働き方を可視化し、HR業務を効率化、社内のコミュニケーションを活性化することを可能にするクラウドツールNippoの紹介です。';

    $page_title = '日報でHR業務を効率化＆生産性向上 | Nippo | LOGIQUEインドネシア';

    $link_canonical = 'https://nippo.id/jp/';

    $add_css = '<link href="/assets/css/home.css" rel="stylesheet">';
    
    $page = '';

    include($_SERVER['DOCUMENT_ROOT'].'/jp/layout/header.php');
?>

<section class="mb-5 hero">
    <div class="container">
        <div class="row">
            <div class="col-11 col-md-7 col-lg-5">
                <div class="hero__text">
                    <h1>日報を通じて精緻な管理会計と適切な人事評価を可能とするクラウドツール。インドネシア法人を対象としています。</h1>
                </div>
                <div class="text-center">
                    <a href="/contact.php" class="btn btn--green btn--round btn--main">デモをリクエスト &nbsp; <img
                            class="arrow-right arrow-right--white" src="/assets/icon/arrow-right.svg" alt="" width="15"></a>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="mb-5 carousel">
    <div class="container">
        <div class="carousel__slide">
            <div class="slide__item" data-label="日報詳細画面">
                <picture>
                    <source srcset="/assets/img/carousel-1.webp" type="image/webp">
                    <img class="w-100" src="/assets/img/carousel-1.jpg" alt="">
                </picture>
            </div>
            <div class="slide__item" data-label="従業員の日報一覧">
                <picture>
                    <source srcset="/assets/img/carousel-2.webp" type="image/webp">
                    <img class="w-100" src="/assets/img/carousel-2.jpg" alt=""></div>
                </picture>
            <div class="slide__item" data-label="作業内容分析画面">
                <picture>
                    <source srcset="/assets/img/carousel-3.webp" type="image/webp">
                    <img class="w-100" src="/assets/img/carousel-3.jpg" alt=""></div>
                </picture>
            <div class="slide__item" data-label="タスク管理">
                <picture>
                    <source srcset="/assets/img/carousel-4.webp" type="image/webp">
                    <img class="w-100" src="/assets/img/carousel-4.jpg" alt=""></div>
                </picture>
        </div>
    </div>
</section>

<section class="description">
    <div class="container">
        <div class="row">
            <div class="col-lg-6 order-2 order-lg-1">
                <p class="fw--600">Nippoはクラウド日報管理ツールです。従業員の生産性を向上するため、作業量とその内容を日報入力を通して正確に把握します。従業員間、またメンバーとスーパーバイザー間のコミュニケーションを促進して、数値に基づいた客観的な人事評価を可能にします。</p>
                <p>プロジェクトマネージャにとってはプロジェクトの原価管理を精緻に行うことも可能にします。</p>
            </div>
            <div class="col-lg-6 order-1 order-lg-2 text-center">
                <picture>
                    <source srcset="/assets/img/nippo_1.webp" type="image/webp">
                    <img src="/assets/img/nippo_1.jpg" alt="" class="img-fluid mb-3 mb-lg-0">
                </picture>
            </div>
        </div>
    </div>
</section>

<section class="why">
    <div class="container text-center">
        <h4 class="section-title">Why Nippo</h4>
        <div class="row why__items">
            <div class="col-md-4 mb-4 mb-md-0">
                <div class="text-center">
                    <img src="/assets/icon/accessible.svg" alt="">
                </div>
                <p>スマホ（ブラウザ）からも利用可能</p>
            </div>
            <div class="col-md-4 mb-4 mb-md-0">
                <div class="text-center">
                    <img src="/assets/icon/summary.svg" alt="">
                </div>
                <p>プロジェクト別、タスク別、従業員別に複数軸で分析可能</p>
            </div>
            <div class="col-md-4 mb-4 mb-md-0">
                <div class="text-center">
                    <img src="/assets/icon/evaluation.svg" alt="">
                </div>
                <p>自己評価＆他者評価を合わせて管理可能</p>
            </div>
        </div>
        <div class="green-circle">
            <a href="/contact.php" class="mx-auto green-circle__link"><img class="arrow-right" src="/assets/icon/arrow-right.svg" alt=""
                    width="15"> NIPPOの活用について相談する</a>
        </div>
    </div>
</section>

<section class="mb-5 problems">
    <div class="container">
        <h4 class="section-title section-title--left-line">このような課題に直面していませんか？</h4>
        <div class="row group-item-border">
            <div class="col-md-6">
                <div class="group-item-border__item">
                    <div class="item__img"><img src="/assets/icon/inefficient.svg" alt=""></div>
                    <p class="item__text">効率的に作業が行われていない。マネージャーは誰がどの作業にどれだけの時間を使ったのか把握できていない。</p>
                </div>
            </div>
            <div class="col-md-6">
                <div class="group-item-border__item">
                    <div class="item__img"><img src="/assets/icon/calendar.svg" alt=""></div>
                    <p class="item__text">従業員に与えたタスクの量と質を定量的に評価する術がないためタスクの量や質がミスマッチであっても気づくことができない。</p>
                </div>
            </div>
            <div class="col-md-6">
                <div class="group-item-border__item">
                    <div class="item__img"><img src="/assets/icon/evaluate.svg" alt=""></div>
                    <p class="item__text">従業員どうしがお互いを評価しあうことができていない。マネージャが介在しない作業である場合に評価ができない。</p>
                </div>
            </div>
            <div class="col-md-6">
                <div class="group-item-border__item">
                    <div class="item__img"><img src="/assets/icon/assessment.svg" alt=""></div>
                    <p class="item__text">評価者の主観でのみ人事評価が行われており、客観的で公平な人事評価ができていない。
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="mb-5 benefit">
    <div class="container">
        <h4 class="section-title section-title--left-line">Nippo導入のメリット</h4>
        <div class="row">
            <div class="col-md-6">
                <picture>
                    <source srcset="/assets/img/benefit.webp" type="image/webp">
                    <img src="/assets/img/benefit.png" alt="" class="img-fluid mb-3 mb-md-0">
                </picture>
            </div>
            <div class="col-md-6">
                <ol class="benefit__list">
                    <li>管理者は、各従業員が行った作業内容を簡単に把握して追跡できます。</li>
                    <li>Nippoを通じて、数値ベースの客観的で公平な評価が可能になります。</li>
                    <li>自己評価と管理者や同僚からの評価が合わせて管理されるため、HRDは従業員に明確に評価を説明できます。</li>
                    <li>同僚の日報にいいねやコメントを残すことができるので、社内コミュニケーションを活性化します。</li>
                    <li>リモートワーカーも、通常のワーカーと同じ基準でパフォーマンスを評価される機会を得ます。</li>
                </ol>
            </div>
        </div>
    </div>
</section>

<?php include($_SERVER['DOCUMENT_ROOT'].'/jp/layout/interest.php') ?>
<?php include($_SERVER['DOCUMENT_ROOT'].'/layout/online-meeting.php') ?>
<?php include($_SERVER['DOCUMENT_ROOT'].'/jp/layout/footer.php') ?>
